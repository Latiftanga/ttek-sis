...
export default function StaffBulkUploadModal({ open, onClose }: Props) {
  ...
  const [result,      setResult]      = useState<StaffBulkUploadResult | null>(null);
  ...
  async function handleDownloadTemplate() {
    ...
    try {
      ...
      a.href = url;
      a.download = `${school?.slug ?? "school"}_staff_import.xlsx`;
      a.click();
  ...
  async function handleUpload() {
    ...
    setUploading(true);
    try {
      const data = await staffApi.bulkUpload(selectedFile);
      setResult(data);
      if (fileInputRef.current) fileInputRef.current.value = "";
      qc.invalidateQueries({ queryKey: [school?.slug, "staff"] });
      if (data.imported > 0) toast.success(data.message);
    } catch (err: unknown) {
      const msg =
        ...
        <div className="flex items-start gap-3 rounded-lg bg-blue-50 p-4 dark:bg-blue-950/40">
          ...
          <div className="flex-1 text-sm">
            ...
            <p className="mt-0.5 text-blue-600 dark:text-blue-400">
              Download the pre-formatted template with gender, title, and status
              dropdowns ready to fill.
            </p>
          </div>
          <Button variant="secondary" size="sm" loading={downloading} onClick={handleDownloadTemplate}>
          ...
          >
            ...
            {selectedFile ? (
              ...
              <>
                ...
                <p className="mt-1 text-xs text-gray-400">or click to browse — .xlsx / .xls</p>
              </>
            )}
          </div>
        )}
          ...
          <div className="space-y-3 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
            ...
            {result.errors.length > 0 && (
              <div className="max-h-48 space-y-2 overflow-y-auto">
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Skipped rows:</p>
                {result.errors.map((e) => (
                  <div
                    key={e.row}
                  ...
                  >
                    ...
                    <div className="text-xs">
                      <span className="font-medium text-red-700 dark:text-red-300">
                        Row {e.row}: {e.data.name}
                        {e.data.staff_number && ` (${e.data.staff_number})`}
                      </span>
